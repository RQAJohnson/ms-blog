import ReactPlayer from 'react-player'
import { Link } from 'react-router-dom';

const Medication = (props) => {
    return (
        <>
        <br></br>
        <br></br>
        <h1 className="text-black text-center display-4">Disease Modifying Drugs - DMDs</h1>
        <div className="container" align="center">
            <p className="lead text-center text-black pt-2 mb-5">What they are? | How they are used? | What if I don't like them?.</p>
        </div>
        
        <div class="row" className="container" align="center" text="black">
      <div class="col-md-8 mx-auto mb-4">
      <ReactPlayer url='https://www.youtube.com/watch?v=8exiUQAiw3w' 
      controls={true}/>        

      </div>
      <hr class="mt-1 mb-1"/>
      <br></br>

      <h2>Injectable Medications: <img width="60"src="data:image/jpeg;base64,"  
      alt="Rebif Injection"/></h2>
            <p><b>What Are They?</b> These injections help modify Relapsing-remitting MS (RRMS) and secondary progressive (SPMS) by giving the host an injection
            which is made up of proteins to help minimise the affects and occurance of new lesions being formed along the spinal cord and brain. These type
            of drugs can be injected yourself with the use of a small hand held device or syringe. 
            <br></br>
            <b>How are they used?</b> An example of an injectable DMD is <i>Rebif</i>. This therapy needs to be administered 3 times a week with at least 48hrs in between 
            each injection (Monday, Wednesday, Friday), rotating the place it was last adminstered (arms, thighs, stomach). The therapy can be given through the use 
            of a RebiSmart Autoinjector.  
            <br></br>
            <b>What if I don't like this treatment?</b> Getting Diagnosed with MS and finding the right treatment for you may take some time, you may prefer taking a pill 
            every few days, or you may prefer an injection. Either way being honest with your support team and conversing with your consultant/nurse is extremley important.
            After all, everyone wants to find what is best for you and your body.</p>
            <p>For further on MS treatments and therapies: <Link to={{ pathname:"https://www.healthline.com/health/multiple-sclerosis/drugs#injections-and-infusions"}} target="NHSwebsite"> Health Line</Link> 
            </p>
            
            <hr class="mt-1 mb-1"/>
      <br></br>

      <h2>Oral Medications: <img width="80"src="https://images.unsplash.com/photo-1584362917137-56406a73241c?ixid=MnwxMjA3fDB8MHxzZWFyY2h8MzZ8fHBpbGxzfGVufDB8fDB8fA%3D%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=60"  
      alt="Cup of Pills"/></h2>
            <p><b>What They Are?</b> Similarily to to the injections, Oral Medications work to lessen the affects of a relapse and occurence of new lesions 
            being formed along the spinal cord and brain, for people who tend to have Relapsing-remitting MS (RRMS) and secondary progressive (SPMS). These 
            therapies come in the forms of pills which can be taken from once per day to a 2 treatment course spread over a 2 week period (Mavenclad).
            <br></br>
            <b>How are they used?</b> An example of this type of treatment is <i>Tecfidera</i>. This therapy is to be taken twice per day with at least a 4hr gap 
            per pill (Once in the morning with a meal and the 2nd pill to be taken in the evening).
            <br></br>
            <b>What if I don't like this treatment?</b> If you find the side effects of a treatment to be too much for you, or you hate the idea of having to 
            take a pill. Don't worry! Speak with your friends; family let them know about your resverations and then ensure that you are in 
            communication with your Neurological team, whether thats your nurse or consultant.</p>
            <p>For further on MS treatments and therapies: <Link to={{ pathname:"https://www.healthline.com/health/multiple-sclerosis/drugs#injections-and-infusions"}} target="NHSwebsite"> Health Line</Link> 
            </p>

            <hr class="mt-1 mb-1"/>
      <br></br>

      <h2>Infused Medications: <img width="30"src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQs8YA5pIrPAmenE--8w-ZkhNnKOZ9XAPhPtQ&usqp=CAU"  alt="Universal Rating"/></h2>
      <p><b>What They Are?</b> Infusion therapies tend to be administered through intravenous infusion methods (IV/Drips). Again these treatments work to reduce the 
      occurence of new lesions along he spinal cord and brain, as well as minimise the disabling affects a relapse may have on a person. Whilst some of the other 
      therapies can be administered by yourself, a few of the infusion treatment have to be done by a medical professional. 
            <br></br>
            <b>How are they used?</b> An example of this type of therapy is <i>Ocrevus</i>. The infusions can take around 3 to 4 hours and are administered in a 
            hospital clinic every 6 months. The first does is usually given 2 weeks aparts from one another in 2 seperate infusions. 
            <br></br>
            <b>What if I don't like this treatment?</b> Depending on the type of MS that you have and have the therapies available, your Neurological team may be able to 
            support in you in finding a treatment that makes you more comfortable and suits your comfort; lifesyle and medical needs. </p>
            <p>For further on MS treatments and therapies: <Link to={{ pathname:"https://www.healthline.com/health/multiple-sclerosis/drugs#injections-and-infusions"}} target="NHSwebsite"> Health Line</Link> 
            </p>

    </div>            
        </>
    );
}

export default Medication; 